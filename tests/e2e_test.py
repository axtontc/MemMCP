...
async def run_e2e_test():
    ...
    server_params = StdioServerParameters(
        ...
        args=[server_script],
        env={"MEMMCP_DB_PATH": "test_e2e.db", "MEMMCP_LOG_PATH": "test_e2e_wal.log", **os.environ}
    )
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            store_res = await session.call_tool("store_memory", arguments={
            ...
            recall_res = await session.call_tool("recall_memories", arguments={
...
if __name__ == "__main__":
    try:
        ...
        # Cleanup
        for file in ["test_e2e.db", "test_e2e.db-shm", "test_e2e.db-wal", "test_e2e_wal.log"]:
